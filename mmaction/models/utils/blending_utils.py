...
class BaseMiniBatchBlending(metaclass=ABCMeta):
    ...
    def __call__(self, imgs: torch.Tensor, batch_data_samples: SampleList,
        ...
        mixed_imgs, mixed_labels, mixup_lam, mixup_indices = self.do_blending(imgs, one_hot_label,
                                                   **kwargs)

        for label_item, mixup_index, sample in zip(mixed_labels, mixup_indices, batch_data_samples):
            sample.set_gt_label(label_item)
            sample.set_field(mixup_lam, 'mixup_lam')
            sample.set_field(batch_data_samples[mixup_index].get('original_gt_label'), 'mixup_label')
...
class MixupBlending(BaseMiniBatchBlending):
    ...
    def do_blending(self, imgs: torch.Tensor, label: torch.Tensor,
        ...
        return mixed_imgs, mixed_label, lam, rand_index
...
class CutmixBlending(BaseMiniBatchBlending):
    ...
    def do_blending(self, imgs: torch.Tensor, label: torch.Tensor,
        ...
        return imgs, label, lam, rand_index
