...
class BaseHead(BaseModule, metaclass=ABCMeta):
    ...
    def predict_by_feat(self, cls_scores: torch.Tensor,
        ...
        num_segs = [d.metainfo.get('num_clips', 0) for d in data_samples]
        if min(num_segs) == max(num_segs):  # either no 'num_clips' member in metainfo or all are equal
            if num_segs[0] == 0:
                num_segs = cls_scores.shape[0] // len(data_samples)
            else:
                num_segs = num_segs[0]
            assert cls_scores.shape[0] == num_segs * len(data_samples)
            cls_scores = self.average_clip(cls_scores, num_segs=num_segs)
        else:
            assert cls_scores.shape[0] == sum(num_segs)
            cls_scores_out = []
            start_index = 0
            for num_segs_i in num_segs:
                cls_scores_out.append(self.average_clip(cls_scores[start_index:start_index+num_segs_i], num_segs=num_segs_i))
                start_index += num_segs_i
            cls_scores = torch.cat(cls_scores_out, dim=0)
    ...
    def average_clip(self,
        ...
        if self.average_clips not in ['score', 'prob', 'best_prob', None]:
            raise ValueError(f'{self.average_clips} is not supported. '
                             f'Currently supported ones are '
                             f'["score", "prob", "best_prob", None]')
        ...
        if self.average_clips is None:
            return cls_scores
        elif self.average_clips == 'prob' or self.average_clips == 'best_prob':
            cls_scores = F.softmax(cls_scores, dim=2)
            if self.average_clips == 'prob':
                cls_scores = cls_scores.mean(dim=1)
            else:  # best_prob
                best_idx = cls_scores.max(dim=2).values.argmax(dim=1)
                cls_scores = cls_scores[torch.arange(cls_scores.shape[0]), best_idx]  # select best_idx over dim 1
