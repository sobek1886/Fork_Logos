...
class PackActionInputs(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if 'label' in results:
            ...
            if len(data_sample.gt_label.shape) == 1 and data_sample.gt_label.shape[0] == 1:
                data_sample.set_field(data_sample.gt_label, 'original_gt_label') # gt_label can be transformed by blending, original class is stored here
        if 'no_event_label' in results:
            data_sample.set_field(results['no_event_label'], 'no_event_label')
            
        if 'start_index' in results and 'end_index' in results:                
            if results['label'] == results.get('no_event_label'):
                gt_start, gt_end = 0, 0
                abs_gt = to_tensor([results['frame_inds'][0], results['frame_inds'][-1]]) # fit frames clip, so that IoU between abs_gt and frame_inds is = 1
            else:
                gt_start = results['gt_start'] / results['clip_len']
                gt_end = (results['gt_end'] - results['clip_len']) / results['clip_len']
                abs_gt = to_tensor([results['start_index'], results['end_index']])

            bound_data = torch.sigmoid(to_tensor([gt_start , gt_end]).float()) * 2 - 1
            frame_inds = to_tensor([results['frame_inds'][0], results['frame_inds'][-1]])
            data_sample.gt_boundaries= bound_data
            data_sample.abs_gt = abs_gt
            data_sample.frame_inds = frame_inds
            
        if 'filename' in results:
           data_sample.set_field(results['filename'], 'filename')
        ...
        # Set meta keys
        img_meta = {k: results[k] for k in results if k in self.meta_keys or k == 'num_clips'}  # num_clips is mandatory as it is used in recognition
