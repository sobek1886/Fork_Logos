...
@TRANSFORMS.register_module()
class RandomAddConnector(BaseTransform):
    """Randomly duplicate frames coming from the sampler, restoring the original clip length. 
    To use RandomAddConnector, place it after the sampler in the practice/test pipeline 
    Args:
        add_ratio: Percentage of extra indices added.
        p: transform probability
    """
    def __init__(self, add_ratio = 0.2, p=0.5):
    ...
    def transform(self, results):
        ...
        frame_idxs = results["frame_inds"]
        frame_idxs = frame_idxs[:int(len(frame_idxs) * (1 - self.add_ratio))]
        clip_len = results["clip_len"]
        frame_idxs = np.sort(np.concatenate([frame_idxs, np.random.choice(frame_idxs, clip_len - len(frame_idxs), replace=True)]))
        results["frame_inds"] = frame_idxs
        return results
    
    
@TRANSFORMS.register_module()
class RandomDropConnector(BaseTransform):
    """Randomly drop frames from indexes coming from sampler with restoring original clip length. 
    Note that this is only available with samplers with unambiguously clear sampling logic (we defined this as having frame_interval).
    To use RandomDropConnector, put it after the sampler in the training/test pipline 
    Args:
        drop_ratio: Percentage of indexes to be deleted.
        p: transform probability
    """
    def __init__(self, drop_ratio = 0.3, p=0.5):
    ...
    def _expand_idx_list_for_drop(self, frame_idxs, frame_interval, new_len, total_frames, drop_ratio):
        #drop the tail of duplicates to avoid duplicate idxs in the middle
        frame_idxs_without_tail = frame_idxs[:(frame_idxs[-1] - frame_idxs[0]) // frame_interval + 1]
        #create list of additional idxs 
        add_idxs = np.arange(frame_idxs_without_tail[-1] + frame_interval, int(frame_idxs_without_tail[-1] + new_len * drop_ratio * frame_interval), frame_interval)
        add_idxs = np.where(add_idxs > total_frames - 1, total_frames - 1, add_idxs)
        #dublicate the last frame if didn't have enough 
        add_idxs = np.concatenate([add_idxs, np.tile(add_idxs[-1], int(new_len) - len(add_idxs) - len(frame_idxs_without_tail))])
        #concat
        frame_idxs = np.concatenate([frame_idxs_without_tail, add_idxs])
        return frame_idxs
    
    def transform(self, results):
        assert results["frame_interval"], "it's impossible to guess the sampling logic"
        if not self.decision():
            return results
        ...
        frame_idxs = results["frame_inds"]
        frame_interval = results["frame_interval"]
        #calculate the new length of the array with indices (so that after removing drop_ratio % of indices, clip_len of indices remains)
        new_len = clip_len / (1 - self.drop_ratio)
        frame_idxs = self._expand_idx_list_for_drop(frame_idxs, frame_interval, new_len, total_frames, self.drop_ratio)
        results["frame_inds"] = np.sort(np.random.choice(frame_idxs, clip_len, replace=False))
        return results


@TRANSFORMS.register_module()
class SpeedChangeConnector(RandomDropConnector):
    """Randomly changes the speed to one of the speeds passed in the speed_change list.
    Note that acceleration is represented by an integer greater than one, 
    while slowdown is only available by a factor of two: speed_change = 0.5.
    Args:
        speed_change (float or list): A value representing the magnitude of speedup/slowdown or a list of such values (value in this case is taken randomly).
        p (float): The probability of the transform function being triggered.
    """
    def __init__(self, speed_change = 2, p=0.5):
        self.p = p
        if not isinstance(speed_change, list):
            self.speed_change = [speed_change]
        else:  
            self.speed_change = speed_change
            
        for value in self.speed_change:
            if value > 1:
                if not isinstance(value, int):
                    raise ValueError("Fractional speedup is not available")
            elif value < 1:
                if value != 0.5:
                    raise ValueError("Only 0.5 slowdown is available")
            else:
                raise ValueError("Incorrect speed change value")
    ...
    def transform(self, results):
        ...
        speed_change = np.random.choice(self.speed_change, 1)
        
        new_len = int(clip_len * speed_change)
        if speed_change > 1:
            assert results["frame_interval"], "it's impossible to guess the sampling logic"
            
            #expand the list with indexes in speed_change times
            frame_idxs = self._expand_idx_list_for_drop(frame_idxs, frame_interval, new_len, total_frames, 1 - 1 / speed_change)
            #select indexes with speed_change step
            idxs = np.arange(0, len(frame_idxs), speed_change)
            frame_idxs = frame_idxs[idxs]
        else:
            #If we want to slow down the video(expand), we need to first shorten it, 
            #so that there will be the same number of frames after the enlargement. 
            #We shorten randomly so that the starting point is not always equal to the first frame
            random_start = np.random.randint(0, len(frame_idxs) - math.ceil(new_len))
            frame_idxs = np.repeat(frame_idxs[random_start : random_start + new_len], int(1 / speed_change))[: len(frame_idxs)]
...
class SampleFrames(BaseTransform):
    ...
    Args:
        ...
        delta_left (int or (int,int)): shift `start_index` to `start_index + delta_left`.
            If 2 values are provided, select random value from the interval
            Defaults to 0.
        delta_right (int or (int,int)): shift `end_index` to `end_index + delta_right`.
            Clip start frame is samled from interval (`start_index + delta_left`, `end_index + delta_right`)
            Video frames are sampled from clip start till end of video with step `frame_interval`
        ...
        out_of_bound_opt (str): The way to deal with out of bounds frame
            ...
            Defaults to 'loop'.
        align_to_end (bool): if video is shorter then required clip, align clip to the end of clip.
            In this case out_of_bound_opt='repeat_last' repeats first frame.
            Defaults to False
    ...
    def __init__(self,
        ...
        assert self.out_of_bound_opt in ['loop', 'repeat_last', 'random']
    ...
    def _get_train_clips(self, num_frames: int,
        ...
        Args:
            num_frames (int): Total number of frames to sample clip starts.
        ...
        if self.keep_tail_frames:
            ...
            elif self.out_of_bound_opt == 'random':
                assert self.num_clips == 1
                clip_offsets = np.random.randint(avg_interval, 0, size=self.num_clips)
            else:
                clip_offsets = np.zeros((self.num_clips, ), dtype=np.int32)
    ...
    def transform(self, results: dict) -> dict:
        ...
        """
        total_frames = results['total_frames']
        if isinstance(self.delta_left, int):
            delta_left = self.delta_left
        else:
            delta_left = np.random.randint(self.delta_left[0], self.delta_left[1])
        if isinstance(self.delta_right, int):
            delta_right = self.delta_right
        else:
            delta_right = np.random.randint(self.delta_right[0], self.delta_right[1])

        if delta_left + delta_right + total_frames < total_frames / self.frame_interval:
            delta_left = delta_left // 2
            delta_right = delta_right // 2

        start_index = min(total_frames - 1, max(0, results['start_index'] - delta_left))
        end_index = max(start_index,
                    min(total_frames - 1,
                       (results.get('end_index') or (total_frames - 1)) + delta_right
            ))  # last frame no
        ...
        clip_offsets = self._sample_clips(end_index - start_index + 1, ori_clip_len)

        if self.align_to_end:
            clip_offsets = end_index + 1 - clip_offsets - ori_clip_len
        else:
            clip_offsets += start_index
        ...
        if self.out_of_bound_opt == 'loop':
            ...
            safe_inds = (0 <= frame_inds) & (frame_inds < total_frames)
            unsafe_inds = 1 - safe_inds
            if self.out_of_bound_opt == 'repeat_last':
                if self.align_to_end:
                    max_idx = np.max(frame_inds)
                    fill_ind = np.min(safe_inds * frame_inds + unsafe_inds * max_idx, axis=1)
                else:
                    fill_ind = np.max(safe_inds * frame_inds, axis=1)
                new_inds = (safe_inds * frame_inds + (unsafe_inds.T * fill_ind).T)
                frame_inds = new_inds
            elif self.out_of_bound_opt == 'random':
                rnd_frame_idxs = np.random.randint(total_frames - 1, size=frame_inds.shape)
                frame_inds = safe_inds * frame_inds + unsafe_inds * rnd_frame_idxs
            else:
        ...
        results["gt_start"] = (results["start_index"] - results["frame_inds"][0]) / self.frame_interval
        results["gt_end"] = (results["end_index"] - results["frame_inds"][0]) / self.frame_interval
        return results
...
class DecordInit(BaseTransform):
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        if not results.get("end_index"):
            results["end_index"] = results["total_frames"] - 1
        results['video_reader'] = container
