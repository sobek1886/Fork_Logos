...
class AlbumentationAugs(BaseTransform):
    """Connector to Albumentations for image augmentations.
    
    Pass a single augmentation or a list of augmentations as the augs parameter.

    Args:
        augs (list[dict] or dict): List of dictionaries or single dictionary representing augmentations from Albumentations. Example of augs passing:
            augs=[dict(type='ImageCompression', quality_lower=65, quality_upper=100, p=0.35)]
    """
    def __init__(self, augs=None):
        if not isinstance(augs, list):
            augs = [augs]
        self.augs = [getattr(A, aug['type'])(**{k:v for k, v in aug.items() if k != 'type'}) for aug in augs]
        
    def transform(self, results):
        imgs = results['imgs']
        
        for aug in self.augs:
            imgs = [aug(image=image)['image'] for image in imgs]
        results['imgs'] = imgs
...
class RandomNoise(BaseTransform):   
    def __init__(self, mode='salt', amount=[0.01, 0.1], p=0.5):
        self.mode = mode
        self.amount = amount
        self.p = p
    def decision(self):
    ...
    def transform(self, results):
        if not self.decision():
            return results
        amount_value = np.random.uniform(*self.amount) if isinstance(self.amount, list) else self.amount
        imgs = results['imgs']
        noised_images = [np.array(255 * util.random_noise(img, mode=self.mode, amount=amount_value), dtype = 'uint8') for img in imgs]
        results['imgs'] = noised_images
        return results


@TRANSFORMS.register_module()
class RandomAug(BaseTransform): 
    """RandomAug data augmentation method based on “RandAugment: 
    Practical automated data augmentation with a reduced search space”. 
    This approach selects with repetitions <num_ops> augmentations from the set of augmentations <augs> .
    An Identity augmentation is also added, which is the absence of applying operations. 
    This augmentation protocol always select a transformation with uniform probability 1 / K, when K is number of available transformations.
    Given <num_ops> transformations for the training image, RandomAug can express K^N potential policies.

    Args:
        augs (list): A set of augmentations from which some will be selected for transformation.
    ...
    class Identity():
        def transform(self, results):
            return results
    def __init__(self, augs, num_ops=2):
        self.num_ops = num_ops
        self.augs = [TRANSFORMS.build(aug) for aug in augs]
        self.augs.append(self.Identity())
    def transform(self, results):
        augs = np.random.choice(self.augs, self.num_ops)
        for aug in augs:
            results = aug.transform(results)
        return results
    
@TRANSFORMS.register_module()
class UniformAug(BaseTransform):  
    """UniformAug data augmentation method based on UniformAugment: "A Search-free Probabilistic Data Augmentation Approach".  
    For each data point (x,y) in the dataset, multiple augmentation operations can be applied. 
    Hyperparameter num_ops is the total number of augmentation transforms applied to the input data point in a sequential manner. 
    Each augmentation transform is selected from augmentation transform set augs.
    Since it is assumed that the distribution of augmentation transform set augs is Uniform, 
    we uniformly sample an operation from the augmentation transform set. 
    Each augmentation transform t has parameter probability of applying the transform p, 
    and the others is the augmentation magnitude, 
    which chose randomly from the configuration (just pass a list with numbers instead of a number).
    Args:
        augs (list): A set of augmentations from which some will be selected for transformation.
        num_ops (int): Number of augmentation transformations to apply sequentially
            Default: 3
    """
    def __init__(self, augs, num_ops=7):
        self.num_ops = num_ops
        self.augs = augs 
    
    def get_uniform_parameters_from_dict(self, dictionary):
        uniform_dict = dict()
        for key,value in dictionary.items():
            if isinstance(value, list) or isinstance(value, tuple):
                uniform_dict[key] = random.uniform(*value)
            elif isinstance(value, dict):
                uniform_dict[key] = self.get_uniform_parameters_from_dict(value)
            else:
    ...
    def transform(self, results):
        augs = np.random.choice(self.augs, self.num_ops)
        for aug in augs:
            uniform_aug = TRANSFORMS.build(self.get_uniform_parameters_from_dict(aug))
            results = uniform_aug.transform(results)
        return results

@TRANSFORMS.register_module()
class Rotation(BaseTransform):
    def __init__(self, degrees=[0, 30], fill=(114, 0, 0)):
        self.degrees = degrees
        self.fill = fill

    def transform(self, results):
        imgs = results['imgs']
        rotation = T.RandomRotation(degrees=self.degrees, fill=self.fill)
        results['imgs'] = [np.array(rotation(Image.fromarray(img))) for img in imgs]
        return results
    
@TRANSFORMS.register_module()
class Perspective(BaseTransform):
    def __init__(self, distortion_scale=0.5, fill=(114, 0, 0)):
        self.distortion_scale = distortion_scale
        self.fill = fill

    def transform(self, results):
        imgs = results['imgs']
        perspective = T.RandomPerspective(distortion_scale=self.distortion_scale, fill=self.fill)
        results['imgs'] = [np.array(perspective(Image.fromarray(img))) for img in imgs]
        return results
    
    
@TRANSFORMS.register_module()
class Affine(BaseTransform):
    def __init__(self, degrees=0.5, fill=(114, 0, 0)):
        self.degrees = degrees
        self.fill = fill

    def transform(self, results):
        imgs = results['imgs']
        affine = T.RandomAffine(degrees=self.degrees, fill=self.fill)
        results['imgs'] = [np.array(affine(Image.fromarray(img))) for img in imgs]
        return results
    

@TRANSFORMS.register_module()
class Sharpness(BaseTransform):
    def __init__(self, sharpness_factor=[0.5, 2], p=0.5):
    ...
    def transform(self, results):
        if not self.decision():
            return results
        imgs = results['imgs']
        sharpness_value = random.choice(self.sharpness_factor) if isinstance(self.sharpness_factor, list) else self.sharpness_factor
        sharpness = T.RandomAdjustSharpness(sharpness_factor=sharpness_value)
        results['imgs'] = [np.array(sharpness(Image.fromarray(img))) for img in imgs]
        return results
...
class Flip(BaseTransform):
    ...
    Args:
        ...
        lazy (bool): Determine whether to apply lazy operation. Default: False.
        blacklisted_labels: if defined, don't apply for enumerated lables. Default: None. 
    """
    ...
    def transform(self, results):
        ...
        flip = not (self.blacklisted_labels and results['label'] in self.blacklisted_labels) and np.random.rand() < self.flip_ratio
...
@TRANSFORMS.register_module()
class Downscale(BaseTransform):
    """Decreases image quality by downscaling and upscaling back.

    Args:
        scale_min (float): lower bound on the image scale. Should be in range (0, 1).
        scale_max (float):  lower bound on the image scale. Should be grater then scale_min and in range (0, 1).
    ...
    class Interpolation:
        def __init__(self, *, downscale = cv2.INTER_NEAREST, upscale = cv2.INTER_NEAREST):
    ...
    def __init__(
        ...
        if isinstance(scale, int):
            ...
            raise ValueError("Expected scale to be int or tuple")
    ...
    def transform(self, results, **params):
        if not self.decision():
            return results
        imgs = results['imgs']
        downscaled_imgs = [self.downscale(
            img,
            scale=random.uniform(self.scale[0], self.scale[1]),
            down_interpolation=self.interpolation.downscale,
            up_interpolation=self.interpolation.upscale) for img in imgs]
        results['imgs'] = downscaled_imgs
    ...
    def downscale(self, img, scale, down_interpolation=cv2.INTER_AREA, up_interpolation=cv2.INTER_LINEAR):
        h, w = img.shape[:2]
        ...
        if need_cast:
            upscaled = self.from_float(np.clip(upscaled, 0, 1), dtype=np.dtype("uint8"))
        return upscaled
    
    def to_float(self, img, max_value=None):
        if max_value is None:
            try:
                max_value = self.MAX_VALUES_BY_DTYPE[img.dtype]
            except KeyError:
                raise RuntimeError(
    ...
    def from_float(self, img, dtype, max_value=None):
        if max_value is None:
            try:
